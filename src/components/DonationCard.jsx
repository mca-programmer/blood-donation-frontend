...
const DonationCard = ({ donation }) => {
  return (
    <div className="card bg-white shadow-md p-4 rounded-lg mb-4">
      ...
      <p className="text-sm text-gray-600">{donation.recipientDistrict}, {donation.recipientUpazila}</p>
      ...
      <p className={`badge ${donation.status === "pending" ? "badge-warning" : donation.status === "inprogress" ? "badge-info" : "badge-success"}`}>
        {donation.status}
      </p>
      <div className="mt-2 flex space-x-2">
        <Link to={`/donation/${donation._id}`} className="btn btn-xs btn-primary">View</Link>
